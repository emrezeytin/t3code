...
const makeServerRuntimeProgram = (input: CliInput) =>
  Effect.gen(function* () {
    ...
    yield* Effect.logInfo("EZ Code running", {
      ...safeConfig,
...
const t3HomeFlag = Flag.string("home-dir").pipe(
  Flag.withDescription("Base directory for all EZ Code data (equivalent to T3CODE_HOME)."),
  Flag.optional,
);
...
export const t3Cli = Command.make("t3", {
  ...
  Command.withDescription("Run the EZ Code server."),
