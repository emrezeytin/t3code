...
const APP_DISPLAY_NAME = isDevelopment ? "EZ Code (Dev)" : "EZ Code (Alpha)";
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("EZ Code failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `EZ Code ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
